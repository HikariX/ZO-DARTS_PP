...
def _generate_z(network, isVaried=False):
    if not isVaried: # Each stage has its own parameters.
        ...
        v2 = network.module.arch_parameters_cell
        length = v1.shape[0] * v1.shape[1] + v2.shape[0] * v2.shape[1] # To record all the lengths, spell them out as a vector.
        rand = torch.randn((length, 1))
        rand = rand / torch.norm(rand)
        z = rand.cuda()
    return z

def _prepare_disturbance(network, epoch, logger, isVaried=False):  # Generate perturbed structural coefficients.
    model_new = deepcopy(network)
    z = _generate_z(network, isVaried)  # Generate perturbation vectors.
    i = 0
    ...
    if isVaried:
        perturbation = torch.reshape(z[:len_arch], model_new.module.arch_parameters.shape) * mu
        perturbation_cell = torch.reshape(z[len_arch:], model_new.module.arch_parameters_cell.shape) * mu

        model_new.module.arch_parameters.data = model_new.module.arch_parameters.data + perturbation.data
        model_new.module.arch_parameters_cell.data = model_new.module.arch_parameters_cell.data + perturbation_cell.data
    else:
...
def _backward_step_ours_cellN(
    ...
    grad_Lval_a = unrolled_model.module.arch_parameters.grad
    grad_Lval_a = torch.cat([torch.flatten(v) for v in grad_Lval_a], dim=0).unsqueeze(dim=1)
    grad_Lval_b = unrolled_model.module.arch_parameters_cell.grad
    grad_Lval_b = torch.cat([torch.flatten(v) for v in grad_Lval_b], dim=0).unsqueeze(dim=1)

    grad_Lval_total = torch.cat([grad_Lval_a, grad_Lval_b], dim=0)  # Check the shape
    ...
    approx_grad_cell = torch.reshape(approx_grad_total[len_arch:], network.module.arch_parameters_cell.shape)

    if network.module.arch_parameters.grad is None:
        network.module.arch_parameters.grad = deepcopy(approx_grad)
        network.module.arch_parameters_cell.grad = deepcopy(approx_grad_cell)
    else:
        network.module.arch_parameters.grad.data.copy_(approx_grad.data)
...
def search_func_ours_F(
    ...
    for step, (base_inputs, base_targets, arch_inputs, arch_targets) in enumerate(
        ...
        if step % rounds == 0:
            ...
            network_perturb, z, mu = _prepare_disturbance(network, epoch_str, logger, isVaried=Flag)
        ...
        if (step + 1) % rounds == 0:
            ...
            if epoch_curr >= epoch_threshold:
                arch_loss, arch_logits = _backward_step_ours_cellN(network, network_perturb, criterion, arch_inputs,
                                                                   arch_targets, z, mu)
            else:
            ...
            if class_num > 5:
                ...
                arch_prec1, arch_prec5 = obtain_accuracy(
                    arch_logits.data, arch_targets.data, topk=(1, class_num)
                )
        ...
        if class_num > 5:
            ...
            base_prec1, base_prec5 = obtain_accuracy(
                logits.data, base_targets.data, topk=(1, class_num)
            )
...
def valid_func(xloader, network, criterion, class_num):
    data_time, batch_time = AverageMeter(), AverageMeter()
    arch_losses, arch_top1, arch_top5 = AverageMeter(), AverageMeter(), AverageMeter()
    network.eval()
    end = time.time()
    with torch.no_grad():
        for step, (arch_inputs, arch_targets) in enumerate(xloader):
            ...
            if class_num > 5:
                ...
                arch_prec1, arch_prec5 = obtain_accuracy(
                    logits.data, arch_targets.data, topk=(1, class_num)
                )
            arch_losses.update(arch_loss.item(), arch_inputs.size(0))
            arch_top1.update(arch_prec1.item(), arch_inputs.size(0))
            arch_top5.update(arch_prec5.item(), arch_inputs.size(0))
...
def main(xargs):
    ...
    logger.log(
        "||||||| {:10s} ||||||| Search-Loader-Num={:}, Valid-Loader-Num={:}, batch size={:}".format(
            xargs.dataset, len(search_loader), len(valid_loader), config.batch_size * 2
        )
    )
    ...
    for epoch in range(start_epoch, total_epoch):
        if epoch % 5 == 0 and epoch != 0:
            ...
            if epoch >= epoch_threshold:
                search_model.cell_temperature *= temperature_decay
        ...
        if epoch == epoch_threshold:  # Start the cell_variable search.
            network.module.initialize_archparam_cells()
            a_optimizer = torch.optim.Adam(
        ...
        valid_a_loss, valid_a_top1, valid_a_top5 = valid_func(
            valid_loader, network, criterion, class_num
        )
        logger.log(
        ...
        with torch.no_grad():
            if search_model.arch_parameters_cell is None:
                logger.log(
                    "arch-parameters :\n{:}".format(
                ...
                logger.log(
                    "arch-parameters :\n{:}, arch-parameters-cell :\n{:}".format(
                        sparsemax(search_model.arch_parameters / search_model.temperature).cpu(),
                        sparsemax(search_model.arch_parameters_cell / search_model.cell_temperature).cpu()
                    )
                )
        if api is not None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="../../nasbench201/dataset/octmnist.npz", help="The path to dataset")
    parser.add_argument(
        "--dataset",
        type=str,
        choices=["cifar10", "cifar100", "ImageNet16-120", "OCTMNIST", "PneumoniaMNIST", "BreastMNIST", "TissueMNIST", "OrganAMNIST", "OrganCMNIST", "OrganSMNIST", "PathMNIST", "DermaMNIST", "BloodMNIST"],
        default="OCTMNIST",
    )
    ...
    parser.add_argument(
        "--save_dir", type=str, default="ZO_S_A_oct+2", help="Folder to save checkpoints and log."
    )
    parser.add_argument(
        "--arch_nas_dataset",
        type=str,
    ...
    parser.add_argument("--temperature_decay", type=str, default="0.9", help="manual temperature decay")
