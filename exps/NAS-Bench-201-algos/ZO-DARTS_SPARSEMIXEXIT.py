            ...
            if search_model.arch_parameters_mix is None:
                ...
                logger.log(
                    "arch-parameters :\n{:}, arch-parameters-mix :\n{:}, arch-parameters-exit :\n{:}".format(
                        sparsemax(search_model.arch_parameters).cpu(),
                        sparsemax(search_model.arch_parameters_mix).cpu(),
                        sparsemax(search_model.arch_parameters_exit).cpu()
                    )
                )
        if api is not None:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_dir", type=str, default="ZO-SPARSEMIXEXIT+2_test", help="Folder to save checkpoints and log."
    )
    parser.add_argument(
        "--arch_nas_dataset",
        type=str,
