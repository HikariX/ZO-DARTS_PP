...
def resource_calculator(network, budget_L, budget_U, threshold=True):
...
#     return (torch.nn.functional.relu(param - budget) + torch.nn.functional.relu(budget + 100000 - param)) / 1000000 # This code is totally wrong!!! Old results should be deleted.
    return (torch.nn.functional.relu(param - budget_U) + torch.nn.functional.relu(budget_L - param)) / 1000000
...
def _backward_step_ours_mixed_penalty(
    ...
    unrolled_model.zero_grad()
    _, unrolled_logits = unrolled_model(arch_inputs)
    unrolled_loss = criterion(unrolled_logits, arch_targets) + lamb * resource_calculator(unrolled_model, budget_L, budget_U)
    # print('*' * 100)
        ...
        if (step + 1) % rounds == 0:
            ...
            if epoch_curr >= epoch_threshold:
                arch_loss, arch_logits = _backward_step_ours_mixed_penalty(network, network_perturb, criterion, arch_inputs,
                                                                   arch_targets, z, mu, budget_L, budget_U)
            else:
...
def main(xargs):
    ...
    mapping = {1:'small', 2:'medium', 3:'large'} # A temporal mapping for the size name since it's complicated to pass string arguments in bash file.
    budget_L = budget_dict[mapping[xargs.size]][xargs.dataset][0]
    budget_U = budget_dict[mapping[xargs.size]][xargs.dataset][1]

    logger.log("The size is {:}, lower bound {:} and upper bound {:}".format(xargs.size, budget_L, budget_U))
    # start training
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="../../nasbench201/dataset/organsmnist.npz",
    ...
    parser.add_argument("--size", type=int, default = 'small', help='resource budget')
