...
def _hessian_vector_product(
    ...
    grads_p = torch.autograd.grad(loss, network.module.get_alphas()) # calculate the gradient of alpha and beta

    for p, v in zip(network.module.get_weights(), vector):
        p.data.sub_(2 * R, v)
    _, logits = network(base_inputs)
    loss = criterion(logits, base_targets)
    grads_n = torch.autograd.grad(loss, network.module.get_alphas()) # as line 39
...
def backward_step_unrolled(
    ...
    dalpha = [a.grad for a in unrolled_model.module.get_alphas()]
    ...
    if network.module.arch_parameters.grad is None:
        network.module.arch_parameters.grad = deepcopy(dalpha[0])
        network.module.arch_parameters_betas.grad = deepcopy(dalpha[1])
    else:
        network.module.arch_parameters.grad.data.copy_(dalpha[0].data)
        network.module.arch_parameters_betas.grad.data.copy_(dalpha[1].data)
...
def main(xargs):
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        with torch.no_grad():
            logger.log(
                "arch-parameters :\n{:}\narch-parameters-betas :\n{:}".format(
                    nn.functional.softmax(search_model.arch_parameters, dim=-1).cpu(),
                    nn.functional.softmax(search_model.arch_parameters_betas, dim=-1).cpu()
                )
            )
        if api is not None:
    ...
    logger.log(
        "PCDARTS : run {:} epochs, cost {:.1f} s, last-geno is {:}.".format(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("PCDARTS")
    ...
    parser.add_argument(
        "--save_dir", type=str, default="PCDARTS2+3", help="Folder to save checkpoints and log."
    )
    parser.add_argument(
        "--arch_nas_dataset",
        type=str,
    ...
    parser.add_argument("--rand_seed", type=int, default=3, help="manual seed")
