...
def kernelV_param_num(weight_edge, input_channel, output_channel, threshold=True): # For the kernel-variable convolution
    return sum(weight_edge[i, 3] * (3 * 3 * input_channel + 1) * output_channel for i in range(6)) # BN of ReLUConvBN included.
...
def resource_calculator(network, budget, threshold=True):
    ...
    stage_cellnum = [0, 4, 8]
    weight_edge = sparsemax(network.module.arch_parameters / network.module.temperature)
    if not threshold:
        ...
        for stage in range(3):
            ...
            conv3 = sum(weight_edge[j, 3] * (3 * 3 * in_channel + 1) * out_channel for j in range(6))
            param = param + (conv1 + conv3) * 3
    ...
    for step, (base_inputs, base_targets, arch_inputs, arch_targets) in enumerate(
        ...
        if (step + 1) % rounds == 0:
            ...
            if epoch_curr >= epoch_threshold:
                arch_loss, arch_logits = _backward_step_ours_penalty(network, network_perturb, criterion, arch_inputs,
