...
def _prepare_disturbance(network, epoch, logger, isMixed=False):  # Generate perturbed structural coefficients.
    ...
    if isMixed:
        ...
        perturbation_mix = torch.reshape(z[30:], model_new.module.arch_parameters_mix.shape) * mu
...
def _backward_step_ours_MixedSize(
    network,
    network_perturb,
    criterion,
    arch_inputs,
    ...
    approx_grad_mix = torch.reshape(approx_grad_total[len_arch:], network.module.arch_parameters_mix.shape)
...
def search_func_ours_F(
    ...
    for step, (base_inputs, base_targets, arch_inputs, arch_targets) in enumerate(
        ...
        if (step + 1) % rounds == 0:
            ...
            if epoch_curr >= epoch_threshold:
                arch_loss, arch_logits = _backward_step_ours_MixedSize(network, network_perturb, criterion, arch_inputs, arch_targets, z, mu)
            else:
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        with torch.no_grad():
            if search_model.arch_parameters_mix is None:
                ...
                logger.log(
                    "arch-parameters :\n{:}, arch-parameters-mix :\n{:}".format(
	                   sparsemax(search_model.arch_parameters).cpu(), sparsemax(search_model.arch_parameters_mix).cpu()
	               )
	           )
        if api is not None:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_dir", type=str, default="ZO-SPARSEMIX+2", help="Folder to save checkpoints and log."
    )
    parser.add_argument(
        "--arch_nas_dataset",
        type=str,
