...
def _prepare_disturbance(network, epoch, logger, isMixed=False):  # Generate perturbed structural coefficients.
    ...
    if isMixed:
        perturbation = torch.reshape(z[:30], model_new.module.arch_parameters.shape) * mu
        perturbation_mix = torch.reshape(z[30:48], model_new.module.arch_parameters_mix.shape) * mu
        perturbation_exit = torch.reshape(z[48:], model_new.module.arch_parameters_exit.shape) * mu
...
#    logger.log(
#        "||||||| {:10s} ||||||| Search-Loader-Num={:}, Valid-Loader-Num={:}, batch size={:}".format(
#            xargs.dataset, len(search_loader), len(valid_loader), 512
#        )
#    )
    ...
    stat(search_model, (3, 32, 32))
#    summary(search_model, input_size=(1, 3, 32, 32),verbose=2,col_names=["input_size","kernel_size","output_size", "num_params", "mult_adds"])
#    print('&' * 100)
#    get_model_complexity_info(search_model, (3, 32, 32), as_strings=True, print_per_layer_stat=True)
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        with torch.no_grad():
            if search_model.arch_parameters_mix is None:
                logger.log(
                    "arch-parameters :\n{:}".format(
                        nn.functional.gumbel_softmax(search_model.arch_parameters, tau=0.5, dim=-1).cpu()
                    )
                )
            else:
                logger.log(
                    "arch-parameters :\n{:}, arch-parameters-mix :\n{:}, arch-parameters-exit :\n{:}".format(
                        nn.functional.gumbel_softmax(search_model.arch_parameters, tau=0.5, dim=-1).cpu(),
                        nn.functional.gumbel_softmax(search_model.arch_parameters_mix, tau=0.5, dim=-1).cpu(),
                        nn.functional.gumbel_softmax(search_model.arch_parameters_exit, tau=0.5, dim=-1).cpu()
                    )
                )
        if api is not None:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_dir", type=str, default="ZO-SPARSEMIXEXITGumbel+2_test", help="Folder to save checkpoints and log."
    )
    parser.add_argument(
        "--arch_nas_dataset",
        type=str,
