...
def backward_step_unrolled_MiLeNAS(
    network,
    criterion,
    base_inputs,
    base_targets,
    ...
    network_tr = deepcopy(network)
    network_val = deepcopy(network)

    network_tr.zero_grad()
    network_val.zero_grad()

    _, base_logits = network_tr(base_inputs)
    base_loss = criterion(base_logits, base_targets)
    base_loss.backward()
    grad_train = network_tr.module.arch_parameters.grad

    _, arch_logits = network_val(arch_inputs)
    arch_loss = criterion(arch_logits, arch_targets)
    arch_loss.backward()
    grad_val = network_val.module.arch_parameters.grad

    dalpha = grad_train + lamb * grad_val
    ...
    return arch_loss.detach(), arch_logits.detach()

def search_func_MiLeNAS(
    xloader,
    network,
    criterion,
    scheduler,
    ...
    for step, (base_inputs, base_targets, arch_inputs, arch_targets) in enumerate(
        ...
        arch_loss, arch_logits = backward_step_unrolled_MiLeNAS(
            network,
            criterion,
            base_inputs,
            base_targets,
...
def main(xargs):
    ...
    logger.log(
        "||||||| {:10s} ||||||| Search-Loader-Num={:}, Valid-Loader-Num={:}, batch size={:}".format(
            xargs.dataset, len(search_loader), len(valid_loader), xargs.batch_size
        )
    )
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        search_w_loss, search_w_top1, search_w_top5 = search_func_MiLeNAS(
            search_loader,
            network,
            criterion,
            w_scheduler,
    ...
    logger.log(
        "MileNAS : run {:} epochs, cost {:.1f} s, last-geno is {:}.".format(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("MileNAS")
    parser.add_argument("--data_path", type=str, default="../../nasbench201/dataset/organamnist.npz",
    ...
    parser.add_argument(
        "--save_dir", type=str, default="MileNAS+2", help="Folder to save checkpoints and log."
    )
    parser.add_argument(
        "--arch_nas_dataset",
        type=str,
