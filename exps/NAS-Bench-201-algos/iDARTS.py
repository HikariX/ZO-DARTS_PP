...
############################################################################################
# iDARTS: Differentiable Architecture Search with Stochastic Implicit Gradients, ICML 2021 #
#############################################################################################
...
def _hessian_vector_product(vector, network, criterion, xloader, eta, r=1e-2):
    ...
    for p, v in zip(network.module.get_weights(),
                    vector):  ####We empirically found that adding noise into supernet could improve the performance
...
def search_func(xloader, network, criterion, scheduler, w_optimizer, a_optimizer, epoch_str, print_freq, logger):
    ...
    for step, (base_inputs, base_targets, arch_inputs, arch_targets) in enumerate(xloader):
        ...
        arch_loss, arch_logits = backward_step_unrolled(network, criterion, xloader, w_optimizer, arch_inputs,
                                                        arch_targets)
        a_optimizer.step()
...
def main(xargs):
    ...
    logger.log(
        "iDARTS : run {:} epochs, cost {:.1f} s, last-geno is {:}.".format(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("iDARTS")
    ...
    parser.add_argument(
        "--save_dir", type=str, default="iDARTS+201+0902", help="Folder to save checkpoints and log."
    )
    parser.add_argument(
        "--arch_nas_dataset",
        type=str,
