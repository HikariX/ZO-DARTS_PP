...
def main(xargs):
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        # DARTS+
        index = torch.argmax(search_model.arch_parameters, dim=1)
        if torch.sum(index==1)>=2:
            break

    logger.log("\n" + "-" * 100)
    # check the performance from the architecture dataset
    logger.log(
        "DARTS+ : run {:} epochs, cost {:.1f} s, last-geno is {:}.".format(
            epoch+1, search_time.sum, genotypes[epoch]
        )
    )
    if api is not None:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_dir", type=str, default="DARTS++2", help="Folder to save checkpoints and log."
    )
    parser.add_argument(
        "--arch_nas_dataset",
        type=str,
