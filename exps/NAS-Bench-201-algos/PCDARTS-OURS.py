...
def _hessian_vector_product(
    ...
    grads_p = torch.autograd.grad(loss, network.module.get_alphas()[0]) # In our implementation, we only calculate the gradient of alpha.

    for p, v in zip(network.module.get_weights(), vector):
        p.data.sub_(2 * R, v)
    _, logits = network(base_inputs)
    loss = criterion(logits, base_targets)
    grads_n = torch.autograd.grad(loss, network.module.get_alphas()[0]) # as line 39
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_dir", type=str, default="PCDARTS+test", help="Folder to save checkpoints and log."
    )
    parser.add_argument(
        "--arch_nas_dataset",
        type=str,
