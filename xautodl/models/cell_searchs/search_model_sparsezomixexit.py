...
class TinyNetworkSPARSEZOMIXEXIT(nn.Module):
    def __init__(
            self, C, N, max_nodes, num_classes, search_space, affine, track_running_stats
    ):
        super(TinyNetworkSPARSEZOMIXEXIT, self).__init__()
        self._C = C
        self._layerN = N
        ...
        with torch.no_grad():
            if self.arch_parameters_mix is None:
                ...
                return "arch-parameters :\n{:}, arch-parameters-mix :\n{:}, arch-parameters-exit :\n{:}".format(
                    sparsemax(self.arch_parameters).cpu(), sparsemax(self.arch_parameters_mix).cpu(),
                    sparsemax(self.arch_parameters_exit).cpu()
                )
    ...
    def forward(self, inputs):
        ...
        if self.arch_parameters_exit is None:
            ...
            exit_weight = sparsemax(self.arch_parameters_exit)
