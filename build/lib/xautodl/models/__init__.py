...
def get_cell_based_tiny_net(config):
    print("get_cell_based_tiny_net")
    if isinstance(config, dict):
        config = dict2config(config, None)  # to support the argument being a dict
    super_type = getattr(config, "super_type", "basic")
    group_names = ["DARTS-V1", "DARTS-V2", "GDAS", "SETN", "ENAS", "RANDOM", "generic", "PCDARTS", "SPARSEZO", "SPARSEZOMIX", "SPARSEZOMIXEXIT", "SPARSEZOMIXEXITGumbel", "SPARSEZOAnneal", "SPARSEZOAnnealCellN", "SPARSEZOAnnealAttention", "SPARSEZOAnnealMIXEXITPenalty"]
