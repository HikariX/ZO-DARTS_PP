...
class NAS201SearchCell(nn.Module):
    ...
    def initialize_mixed_op(self):
        for i in range(1, self.max_nodes):
            ...
            for j in range(i):
                node_str = "{:}<-{:}".format(i, j)
                self.edges[node_str][3].initialize_mixed_weight() # Call the function to initialize mixed op's weight.

    def forward(self, inputs, weightss, weightss_mix=None):
        nodes = [inputs]
        for i in range(1, self.max_nodes):
            inter_nodes = []
            for j in range(i):
                ...
                if weightss_mix is None:
                    weights_mix = None
                else:
                    weightss_mix = sparsemax(weightss_mix)
#                    print('&' * 50, 'In forward of search_cells, ', weightss_mix)
                    weights_mix = weightss_mix[self.edge2index[node_str]] # Choose one edge's weight
#                    print('&' * 50, 'In forward of search_cells, ', weights.shape, weights_mix.shape)
                inter_nodes.append(
                    sum(
                        layer(nodes[j], weights_mix) * w for layer, w in zip(self.edges[node_str], weights)
                    )
                )
